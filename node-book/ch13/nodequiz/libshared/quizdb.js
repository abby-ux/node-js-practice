...
import pg from 'pg'; // this library handles PostgreSQL db connectivity

// data type parsers
// pg returns all record fields as strings, then the parser parses them to the correct type
...
// connection pool
// provides a resuable set of db connection clients you can check out, use, release, reuse
// ex of how to use: could write code for a SQL query returns all records from a 'question' table using one of the pool connections
...
/**
 * creatiing individual SQL commands can be annoying to this module instead has the following
 * private methods, which make development easier.
 */
...
export async function playerCreate( game_id, name ) {
  ...
  return await dbInsert({
    ...
    values: { game_id, name }, // values object with name/pair values 
    return: 'id' // fetch the id of the added record 
  });
...
// database INSERT
// return the added id or false when an error occurs
// pass object: { table: <tablename>, values: <{ n1: v1,... }>, return: <field> }
async function dbInsert(ins) {

  // values that will create the SQL string
  const
    ret = ins.return ? ` RETURNING ${ ins.return }` : '',
    key = Object.keys( ins.values ), // defines array of property names extracted from the values object
    sym = key.map( (v,i) => `$${i + 1}` ), // defines array of $1 to $N strings, matches the number of items in the key array
  ...
  try {
    ...
    // run insert
    const i = await client.query(sql, Object.values( ins.values )); // passes property values into SQL string
